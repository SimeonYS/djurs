...
class DjursPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('djurs.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `djurs`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM djurs WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO djurs (date, title, link, content)"
